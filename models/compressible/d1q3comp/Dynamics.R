...
AddField(name="s",     dx=c(-2,2), dy=c(0,0), group="entropy", comment="Entropy")
AddField(name="a1xx",  dx=c( 0,0), dy=c(0,0), group="tensor",  comment="Second-order off-equilibrium moment")
AddField(name="Exx",   dx=c( 0,0), dy=c(0,0), group="psi",     comment="E in psi_i = H_i E, Eq. (4.3.11)")
...
AddStage("PsiEQ", "CalcEqnPsi",
    load = FALSE,
    save = Fields$group %in% c("feq", "psi"),
    can.overwrite = TRUE)
